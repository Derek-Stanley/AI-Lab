...
def playerMove():
    pos = input('Select a position to place \'X\' (1 - 9):  ')
    while True:
        try:
            pos = int(pos)
            while not 1 <= pos <= 9 or not isFree(pos):
                pos = int(input('Enter a valid position (1 - 9):  '))
            makeMove('X', pos)  
            break
        except:
            pos = input('Enter a valid position (1 - 9):  ')
...
def main():
    ...
    while notFull(board):
        ...
        if isWinner(board, 'O'):
            print('Noooooo, the computer won!!!')
            break
